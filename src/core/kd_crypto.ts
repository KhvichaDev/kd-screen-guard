...
export async function kd_sha256(data: string): Promise<string> {
    if (!data) return '';
...
export async function kd_pbkdf2(password: string, salt: string, iterations: number = 100000): Promise<string> {
...
function kd_rotr(n: number, b: number): number {
    return (n >>> b) | (n << (32 - b));
}
