...
self.onmessage = async function(e) {
    ...
    try {
        if (data.type === 'PBKDF2') {
            const hash = await kd_pbkdf2(data.password, data.salt, data.iterations);
            self.postMessage({ id: data.id, success: true, hash });
        } else if (data.type === 'SHA256') {
            const hash = await kd_sha256(data.data);
            self.postMessage({ id: data.id, success: true, hash });
        }
    } catch (err) {
...
async function kd_pbkdf2(password, salt, iterations = 100000) {
    if (!password || !salt) return '';
    if (self.crypto && self.crypto.subtle) {
        try {
            const encoder = new TextEncoder();
...
function kd_jsSha256(str) {
    ...
    const K = [
        ...
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0.4a7484aa, 0x5cb0a9dc, 0x76f988da,
...
function kd_rotr(n, b) { return (n >>> b) | (n << (32 - b)); }
`;
...
function kd_stringToBuffer(str) {
  const encoder = new TextEncoder();
...
function useScreenGuard(options = {}) {
  const [isLocked, setIsLocked] = (0, import_react2.useState)(false);
  const guardRef = (0, import_react2.useRef)(null);
  ...
  const updateOptions = (0, import_react2.useCallback)(async (newOptions) => {
    if (guardRef.current) {
